...
Note: In the current version of MOLUSCE, it is essential that the initial and final LULC rasters contain the same set of unique class values. For example, if the initial raster includes the values 1, 2, 3, and 4, then the final raster must also contain exactly these values: 1, 2, 3, and 4. If a new class value appears or an existing one disappears during the transition, MOLUSCE will not be able to process the data. As a workaround, users can artificially adjust the sets of unique values to match. One approach is to use the free \textbf{Serval} plugin, which allows editing of raster pixel values. You can, for instance, add a single pixel with the missing class value in a corner of the raster. This small change will not significantly affect the transition analysis but will ensure that MOLUSCE can process your data.\bigskip

Note: To assist with dataset preparation, MOLUSCE includes dedicated tools in the QGIS Processing Toolbox. These tools are located under the MOLUSCE group. Prepare raster dataset for MOLUSCE — Resamples and reprojects any raster so that it matches the spatial domain (CRS, extent, resolution, and pixel grid) of the reference raster, typically the input land cover layer. Prepare vector dataset for MOLUSCE — Converts vector data into raster format in two ways, Rasterization of features (with optional buffering) for presence-based variables, Generation of proximity rasters for distance-based predictor variables. These tools streamline the creation of spatial variables required for land-cover change modelling within the MOLUSCE workflow.
